...
function osf_add_back_button_inspector() {
	...
	?>
		<script type="text/javascript">
            (function($){
                $( '#tmpl-elementor-panel-footer-content' ).get( 0 ).remove();
            })(jQuery);
        </script>
        <script type="text/template" id="tmpl-elementor-panel-footer-content">
            <div id="elementor-panel-footer-back-to-admin" class="elementor-panel-footer-tool elementor-leave-open tooltip-target" data-tooltip="<?php esc_attr_e( 'Back', 'opalmegamenu' ); ?>">
				<i class="fa fa-arrow-left" aria-hidden="true"></i>
			</div>
			<div id="elementor-panel-footer-responsive" class="elementor-panel-footer-tool">
				<i class="eicon-device-desktop tooltip-target" aria-hidden="true" data-tooltip="<?php esc_attr_e( 'Responsive Mode', 'opalmegamenu' ); ?>"></i>
				<span class="elementor-screen-only">
					<?php echo __( 'Responsive Mode', 'opalmegamenu' ); ?>
				</span>
				<div class="elementor-panel-footer-sub-menu-wrapper">
					<div class="elementor-panel-footer-sub-menu">
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="desktop">
							...
							<span class="elementor-title"><?php echo __( 'Desktop', 'opalmegamenu' ); ?></span>
							<span class="elementor-description"><?php echo __( 'Default Preview', 'opalmegamenu' ); ?></span>
						</div>
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="tablet">
							...
							<span class="elementor-title"><?php echo __( 'Tablet', 'opalmegamenu' ); ?></span>
							<?php $breakpoints = Elementor\Core\Responsive\Responsive::get_breakpoints(); ?>
							<span class="elementor-description"><?php echo sprintf( __( 'Preview for %s', 'opalmegamenu' ), $breakpoints['md'] . 'px' ); ?></span>
						</div>
						<div class="elementor-panel-footer-sub-menu-item" data-device-mode="mobile">
							...
							<span class="elementor-title"><?php echo __( 'Mobile', 'opalmegamenu' ); ?></span>
							<span class="elementor-description"><?php echo __( 'Preview for 360px', 'opalmegamenu' ); ?></span>
						</div>
					</div>
				</div>
			</div>
			<div id="elementor-panel-footer-history" class="elementor-panel-footer-tool elementor-leave-open tooltip-target" data-tooltip="<?php esc_attr_e( 'History', 'opalmegamenu' ); ?>">
				<i class="fa fa-history" aria-hidden="true"></i>
				<span class="elementor-screen-only"><?php echo __( 'History', 'opalmegamenu' ); ?></span>
			</div>
			<div id="elementor-panel-saver-button-preview" class="elementor-panel-footer-tool tooltip-target" data-tooltip="<?php esc_attr_e( 'Preview Changes', 'opalmegamenu' ); ?>">
				<span id="elementor-panel-saver-button-preview-label">
					<i class="fa fa-eye" aria-hidden="true"></i>
					<span class="elementor-screen-only"><?php echo __( 'Preview Changes', 'opalmegamenu' ); ?></span>
				</span>
			</div>
			<div id="elementor-panel-saver-publish" class="elementor-panel-footer-tool">
				<button id="elementor-panel-saver-button-publish" class="elementor-button elementor-button-success elementor-saver-disabled">
					...
					<span id="elementor-panel-saver-button-publish-label">
						<?php echo __( 'Publish', 'opalmegamenu' ); ?>
					</span>
				</button>
			</div>
			<div id="elementor-panel-saver-save-options" class="elementor-panel-footer-tool" >
				<button id="elementor-panel-saver-button-save-options" class="elementor-button elementor-button-success tooltip-target elementor-saver-disabled" data-tooltip="<?php esc_attr_e( 'Save Options', 'opalmegamenu' ); ?>">
					<i class="fa fa-caret-up" aria-hidden="true"></i>
					<span class="elementor-screen-only"><?php echo __( 'Save Options', 'opalmegamenu' ); ?></span>
				</button>
				<div class="elementor-panel-footer-sub-menu-wrapper">
					...
					<div class="elementor-panel-footer-sub-menu">
						<div id="elementor-panel-saver-menu-save-draft" class="elementor-panel-footer-sub-menu-item elementor-saver-disabled">
							...
							<span class="elementor-title"><?php echo __( 'Save Draft', 'opalmegamenu' ); ?></span>
						</div>
						<div id="elementor-panel-saver-menu-save-template" class="elementor-panel-footer-sub-menu-item">
							...
							<span class="elementor-title"><?php echo __( 'Save as Template', 'opalmegamenu' ); ?></span>
						</div>
					</div>
				</div>
			</div>
...
function osf_load_menu_data() {
	...
	if ( ! wp_verify_nonce( $nonce, 'osf-menu-data-nonce' ) || ! $menu_id ) {
		wp_send_json( array(
				'message' => __( 'Access denied', 'opalmegamenu' )
			) );
	}
	...
	if( isset($_POST['istop']) && absint($_POST['istop']) == 1  ){
		if ( class_exists( 'Elementor\Plugin' ) ) {
			if( isset($data['enabled']) && $data['enabled'] ){
				...
				$url = admin_url();
				$data['edit_submenu_url'] = add_query_arg( array( 'opal-menu-createable' => 1, 'menu_id' => $menu_id ), $url );
			}	
		}
	}	
					...
					<% } %>
						<div class="osf-modal-content">
							...
							<% } else { %>
								<div class="form-group">
									<label for="icon_color"><?php _e( 'Menu Description', 'opalmegamenu' ) ?></label>
									<input type="text" name="opal-menu-item[description]" value="<%= data.description %>" class="input" id="menu_description" />
								</div>
								<div class="form-group">
									<label for="icon"><?php _e( 'Icon', 'opalmegamenu' ) ?></label>
									<select id="icon" name="opal-menu-item[icon]" class="form-control icon-picker">
										<option value=""<%= data.icon == '' ? ' selected' : '' %>><?php echo esc_html( "No Use", "opalmegamenu" ) ?></option>
								...
								<div class="form-group submenu-setting toggle-select-setting">
									<label><?php _e( 'Mega Submenu Enabled', 'opalmegamenu' ) ?></label>
									<select name="opal-menu-item[enabled]" class="opal-input-switcher opal-input-switcher-true" data-target=".submenu-width-setting">
										<option value="1" <%= data.enabled == 1? 'selected':'' %>> <?php _e( 'Yes', 'opalmegamenu' ) ?></opttion>
										<option value="0" <%= data.enabled == 0? 'selected':'' %>><?php _e( 'No', 'opalmegamenu' ) ?></opttion>
									</select>
									<button id="edit-megamenu" class="button button-primary button-large">
										<?php _e( 'Edit Megamenu Submenu', 'opalmegamenu' ) ?>
									</button>
								</div>

								<div class="form-group submenu-width-setting toggle-select-setting" style="display: none">
									<label><?php _e( 'Sub Megamenu Width', 'opalmegamenu' ) ?></label>
									<select name="opal-menu-item[customwidth]" class="opal-input-switcher opal-input-switcher-true" data-target=".submenu-subwidth-setting">
										<option value="1" <%= data.customwidth == 1? 'selected':'' %>> <?php _e( 'Yes', 'opalmegamenu' ) ?></opttion>
										<option value="0" <%= data.customwidth == 0? 'selected':'' %>><?php _e( 'Set FullWidth', 'opalmegamenu' ) ?></opttion>
										<option value="2" <%= data.customwidth == 2? 'selected':'' %>><?php _e( 'Set Stretch Width', 'opalmegamenu' ) ?></opttion>
									</select>
								</div>

								<div class="form-group submenu-width-setting submenu-subwidth-setting toggle-select-setting" style="display: none">
									<label for="menu_subwidth"><?php _e( 'Sub Mega Menu Max Width', 'opalmegamenu' ) ?></label>
						...
						<% if ( data.is_loading !== true && data.edit_submenu !== true ) { %>
							<div class="osf-modal-footer">
								...
								<button type="submit" class="button button-primary button-large menu-save pull-right"><%= osf_memgamnu_params.i18n.submit %></button>
							</div>
						<% } %>
					<% if ( data.edit_submenu !== true && data.is_loading !== true ) { %>
						</form>
